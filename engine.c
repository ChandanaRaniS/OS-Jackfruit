...
while (1) {
    FILE *fp = fopen("/tmp/engine_cmd.txt", "r");

    if (fp != NULL) {
        char input[100];

        if (fgets(input, sizeof(input), fp) != NULL) {
            printf("Supervisor received: %s", input);
        }

        fclose(fp);
        remove("/tmp/engine_cmd.txt");
    }
...
static int cmd_start(int argc, char *argv[])
{
    ...
    FILE *fp = fopen("/tmp/engine_cmd.txt", "w");
    ...
    printf("Sent command to supervisor\n");
